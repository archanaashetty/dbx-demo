...
```bash
conda create -n cicd_demo_project python=3.9
conda activate cicd_demo_project
```
...
```
This command will create your personal repository under `/Repos/<username>/cicd_demo_project`.
...
- To trigger the release pipeline, get the current version from the `cicd_demo_project/__init__.py` file and tag the current code version:
```
